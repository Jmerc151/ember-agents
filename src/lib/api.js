...
const API_KEY = import.meta.env.VITE_API_KEY || localStorage.getItem('ember_api_key') || ''
...
async function request(path, options = {}) {
  ...
  if (res.status === 401) {
    ...
    const key = prompt('Enter your Ember API key:')
    if (key) {
      localStorage.setItem('ember_api_key', key)
